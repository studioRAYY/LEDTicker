...
def resource_path(*parts):
    """Absolute Pfade, auch im PyInstaller-Bundle."""
    base = getattr(sys, "_MEIPASS", os.path.abspath("."))
    return os.path.join(base, *parts)

def resolve_ffmpeg():
    """FFmpeg: zuerst gebündeltes Binary, dann PATH."""
...
def build_rois_from_preset(preset) -> List[PortROI]:
    """Zickzack-Layout aus Preset; wenn fehlend, default erstellen."""
...
def draw_roi_overlay(img: QImage, rois: List[PortROI]) -> QImage:
    ...
    for idx, port in enumerate(rois):
        ...
        for i,(x,y,w,h) in enumerate(port.rects):
            p.drawRect(x,y,w,h); p.drawText(x+4,y+18,f"{port.port_id}:{i+1}")
    p.end()
    return out

# ----------------------------- Master Strip -----------------------------
class MasterStrip:
    """Einzeiliger Ticker (H = module_h); liefert Quell-Kacheln für Module."""
    ...
    def rebuild(self):
        ...
        p.setPen(QColor(*self.text_rgb)); p.setFont(self.font)
        baseline = (h + fm.ascent() - fm.descent()) // 2
        p.drawText(0, baseline, self.text); p.end()
        self.double = QImage(self.text_w*2, h, QImage.Format_RGB888)
        p2 = QPainter(self.double)
        p2.drawImage(0,0,self.single); p2.drawImage(self.text_w,0,self.single); p2.end()
...
class Scheduler:
    ...
    def pick_content_name(self, now: Optional[datetime.datetime] = None) -> Optional[str]:
        ...
        t = now.time(); wd = now.weekday()
        for e in self.entries:
            if e.get("type") == "date" and e.get("date") == now.date().isoformat():
                if in_range(t, parse_time(e["start"]), parse_time(e["end"])): return e.get("content")
        for e in self.entries:
...
class Preview(QWidget):
    def __init__(self, w: int, h: int):
        super().__init__(); self.setMinimumSize(800, 320)
    ...
    def paintEvent(self, _):
        ...
        p.drawImage((r.width()-scaled.width())//2,(r.height()-scaled.height())//2, scaled); p.end()

class OutputWindow(QWidget):
    def __init__(self, w: int, h: int):
        super().__init__(); self.setWindowTitle("Ticker Output")
...
class Main(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle("Studio Rayy — Multi-Content Ticker (Master-Strip)")
        self.cfg = CFG(); self.offset = 0.0

        # Preset robust laden (Fallback: in-code Default)
        self.preset = self.load_initial_preset()

        self.module_w = self.preset["module"]["w"]; self.module_h = self.preset["module"]["h"]
        ...
        self.dest_sequence = []
        for pid in self.concat_order: self.dest_sequence += port_map[pid].rects
        ...
        self.crossfade_active = False; self.crossfade_start: Optional[datetime.datetime] = None
        ...
        for name in self.contents.keys(): self.contents_list.addItem(name)
        v1.addWidget(QLabel("Contents (mehrere Presets für Text+Style):")); v1.addWidget(self.contents_list, 1)

        form = QHBoxLayout(); left = QVBoxLayout()
        ...
        left.addLayout(colorrow); form.addLayout(left, 1)
        right = QVBoxLayout()
        ...
        v2.addWidget(QLabel("Scheduler (GUI):")); v2.addWidget(self.tbl, 1)
        rowb = QHBoxLayout()
        ...
        rowb.addWidget(self.btn_add_row); rowb.addWidget(self.btn_del_row); v2.addLayout(rowb)
        ...
        self.full_btn = QPushButton("Fullscreen An/Aus"); self.exp_btn = QPushButton("Export MP4")
        ...
        v3.addLayout(ctl); self.preview = Preview(self.cfg.width, self.cfg.height); v3.addWidget(self.preview, 1)
        ...
        if self.contents_list.count() > 0: self.contents_list.setCurrentRow(0)
    ...
    def on_select_content(self, cur: QListWidgetItem, _):
        if not cur: return
        c = self.contents.get(cur.text()); if not c: return
        self.c_name.setText(c.name); self.c_text.setText(c.text)
        self.c_font.setCurrentFont(QFont(c.font_family)); self.c_size.setValue(float(c.font_pt))

    def pick_content_color(self, text=True):
        name = self.c_name.text().strip()
        if name not in self.contents: return
    ...
    def add_content(self):
        ...
        self.contents[base.name] = base; self.contents_list.addItem(base.name)
        self.contents_list.setCurrentRow(self.contents_list.count()-1)

    def dup_content(self):
        cur = self.contents_list.currentItem(); if not cur: return
        old = self.contents[cur.text()]
        i, new_name = 1, old.name + "_copy"
        while new_name in self.contents: i += 1; new_name = f"{old.name}_copy{i}"
        self.contents[new_name] = ContentItem(new_name, old.text, old.font_family, old.font_pt, old.text_rgb, old.bg_rgb)
        self.contents_list.addItem(new_name); self.contents_list.setCurrentRow(self.contents_list.count()-1)

    def del_content(self):
        cur = self.contents_list.currentItem(); if not cur: return
        name = cur.text(); self.contents.pop(name, None)
        self.contents_list.takeItem(self.contents_list.currentRow())

    def make_strip(self, name: Optional[str]) -> Optional[MasterStrip]:
        if not name or name not in self.contents: return None
    ...
    def load_scheduler_into_table(self):
        ...
        for e in self.scheduler.entries: self.add_sched_row(e)

    def add_sched_row(self, e: dict = None):
        r = self.tbl.rowCount(); self.tbl.insertRow(r)
        type_cb = QComboBox(); type_cb.addItems(["daily","date"])
        if e: type_cb.setCurrentText(e.get("type","daily")); self.tbl.setCellWidget(r, 0, type_cb)
    ...
    def fullscreen_toggle(self):
        ...
        if self.out_win.isVisible():
            ...
            if ret == QMessageBox.Yes: self.out_win.hide()
        else:
            if len(screens) < 2:
                QMessageBox.information(self, "Info", "Kein zweiter Bildschirm erkannt."); return
            g = screens[1].geometry(); self.out_win.setGeometry(g); self.out_win.showFullScreen()

    def composite_crossfade(self, a: QImage, b: QImage, alpha: float) -> QImage:
        out = QImage(a.size(), QImage.Format_RGB888)
        p = QPainter(out); p.drawImage(0,0,a); p.setOpacity(alpha); p.drawImage(0,0,b); p.setOpacity(1.0); p.end(); return out

    def choose_target_content(self) -> Tuple[Optional[str], Optional[dict]]:
        now = datetime.datetime.now()
        for e in self.scheduler.entries:
            if e.get("type") == "date" and e.get("date") == now.date().isoformat():
                if in_range(now.time(), parse_time(e["start"]), parse_time(e["end"])): return e.get("content"), e
        for e in self.scheduler.entries:
    ...
    def tick(self):
        ...
        if desired != self.current_content_name and not self.crossfade_active:
            ...
            if trans == "cut":
                self.current_content_name = desired; self.strip_curr = self.make_strip(self.current_content_name)
            else:
                self.next_content_name = desired; self.strip_next = self.make_strip(self.next_content_name)
                self.crossfade_ms = int((entry or {}).get("fade_ms", 800))
                self.crossfade_active = True; self.crossfade_start = datetime.datetime.now()

        # Bewegung
        self.cfg.speed_px_per_frame = float(self.speed.value()); self.offset += self.cfg.speed_px_per_frame

        base_curr = self.render_mapped_frame(self.strip_curr, self.offset); frame = base_curr
        if self.crossfade_active and self.strip_next is not None and self.crossfade_start is not None:
            ...
            if elapsed >= self.crossfade_ms:
                self.current_content_name = self.next_content_name; self.strip_curr = self.strip_next
                self.strip_next = None; self.crossfade_active = False; self.crossfade_start = None
    ...
    def load_preset(self):
        ...
        if not path: return
        try:
            p = json.load(open(path,"r")); self.apply_preset(p)
    ...
    def apply_preset(self, p: dict):
        self.preset = p
        self.module_w = p.get("module",{}).get("w",128); self.module_h = p.get("module",{}).get("h",256)
        ...
        for c in p["contents"]:
            item = ContentItem(c["name"], c["text"], c["font_family"], int(c["font_pt"]),
                               tuple(c["text_rgb"]), tuple(c["bg_rgb"]))
            self.contents[item.name] = item; self.contents_list.addItem(item.name)
        if self.contents_list.count() > 0: self.contents_list.setCurrentRow(0)
        ...
        self.strip_curr = self.make_strip(self.current_content_name); self.strip_next = None
        self.crossfade_active = False; self.crossfade_start = None; self.offset = 0.0

    def commit_current_content_edits(self):
        name = self.c_name.text().strip(); if not name: return
        ...
        if cur_item and name != cur_item.text():
            ...
            old_name = cur_item.text(); c = self.contents.pop(old_name); c.name = name
            self.contents[name] = c; cur_item.setText(name)
        if name not in self.contents:
            self.contents[name] = ContentItem(name, "", "Arial", 72, (255,255,255), (0,0,0))
        c = self.contents[name]
        c.text = self.c_text.text(); c.font_family = self.c_font.currentFont().family(); c.font_pt = int(self.c_size.value())

    # -------- Export --------
    def export_video(self):
        ffmpeg = resolve_ffmpeg()
        if not ffmpeg or not os.path.isfile(ffmpeg):
            QMessageBox.critical(self, "FFmpeg fehlt", "FFmpeg wurde nicht gefunden/bündelt."); return
        ...
        period = self.strip_curr.period_frames(int_speed); secs = period / self.cfg.fps
        ...
        try:
            proc = subprocess.Popen(cmd, stdin=subprocess.PIPE); local = 0.0
            for _ in range(period):
                frame = self.render_mapped_frame(self.strip_curr, local)
                ptr = frame.bits(); ptr.setsize(frame.byteCount()); proc.stdin.write(bytes(ptr))
                local += int_speed
